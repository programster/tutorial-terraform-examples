...
# Add a secret password/token for use in a script/command.
resource "rundeck_password" "my_secret" {
    path         = "project/${var.project_slug_name}/my_secret" # this path will be prefixed with "keys" when looking in rundeck in key storage.
    password     = var.my_secret_value
}
...
resource "rundeck_job" "my_job" {
    ...
    # Configure an option in order to pass the secret in rundeck to the inline-script that will execute on the node.
    option {
        # Specify the name for the option. This will be how it is referred to from inside the script.
        name = "my_inline_script_secret"

        # Set obscure_input so wont be shown in logs etc. This must be set to true when passing rundeck_password items.
        obscure_input = true

        # Hide this from the user as they don't need to know about it or edit it when running the job.
        hidden = true

        # Specify the path to the secret that is being passed.
        storage_path = "keys/${rundeck_password.my_secret.path}"

        # Specify that this is exposed to scripts, so our inline-script or commands.
